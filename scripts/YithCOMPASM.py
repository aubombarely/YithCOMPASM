...
<body>
  ...
  <div id="main">
    ...
    <div id="sidepanel">
      ...
      <div class="file-status" id="exportStatus"></div>
    </div>
  </div>
  <div id="tooltip"></div>
...
document.getElementById("basketDownloadIds").addEventListener("click", () => {
  if (!basket.size) {
    ...
    exportStatus.textContent = "Basket is empty — click alignment lines to add sequences first.";
    return;
  }
...
function triggerDownload(content, filename, mime) {
  try {
    ...
    exportStatus.textContent = "Download failed: " + err.message +
        " (if you're viewing this inside an embedded preview, open the HTML file directly in a browser tab and try again)";
  }
}
...
document.getElementById("basketDownloadFasta").addEventListener("click", () => {
  ...
  if (!parts.length) {
    exportStatus.style.color = "#E8604C";
    exportStatus.textContent = "None of the basketed sequences were found — missing: " + missing.join(", ");
    return;
  }
  triggerDownload(parts.join(""), "YithCOMPASM_basket_sequences.fasta", "text/plain");
  if (missing.length) {
    exportStatus.style.color = "#F5A623";
    exportStatus.textContent += " (skipped " + missing.length + ": " + missing.join(", ") + ")";
  }
});
...
function renderLabels(minLen) {
  ...
  // Real genome coordinates run into the tens of millions of viewBox units, so
  // scaling font-size *up* into data space (e.g. ~179,000 units to read as 10px
  // after the viewBox transform) hits practical font-size caps in real browser
  // engines and silently fails to render at all. Instead keep font-size a small,
  // constant, real value and counter-scale each label's own coordinate system via
  // a per-element transform — the same idea as vector-effect:non-scaling-stroke,
  // applied manually since there is no non-scaling-size equivalent for text.
  const invX = 1 / pxPerUnitX();
